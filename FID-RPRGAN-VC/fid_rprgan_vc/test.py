...
from fid_rprgan_vc.model import Generator, Discriminator
...
class MaskCycleGANVCTesting(object):
    ...
    def __init__(self, args):
        ...
        self.converted_audio_dir = os.path.join(args.save_dir, args.name, 'convertedPONO601_audio')
        self.original_audio_dir=os.path.join(args.save_dir,args.name, 'originalPONO601_audio')
    ...
    def test(self):
        for i, sample in enumerate(tqdm(self.test_dataloader)):
            ...
            if self.model_name == 'generator_A2B':
                #real_A = torch.cat([sample[0],sample[1]],dim=0)
                #print(len(real_A))
                real_A=sample
                ...
                save_path_orig = os.path.join(self.original_audio_dir,
                ...
                save_path_orig = os.path.join(self.original_audio_dir_audio_dir,
